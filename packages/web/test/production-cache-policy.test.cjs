...
describe('production PWA cache policy', () => {
  ...
  it('keeps build-specific chat documents out of static output and the precache manifest', () => {
    ...
    assert.match(
      artifacts.serviceWorker,
      /registerRoute\(\(\{request:\w+\}\)=>"navigate"===\w+\.mode,new [\w$.]+\.NetworkOnly\(\{cacheName:"pages"/,
      'online navigation must never fall back to an older build-specific document shell',
    );
