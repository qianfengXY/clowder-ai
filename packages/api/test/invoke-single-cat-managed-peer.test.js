...
for (const sourceKind of ['a2a', 'owner']) {
  test(`${sourceKind} invocation reaches the peer provider without parent implementation binding`, async (t) => {
    ...
    const source = {
      ...
      userId: 'owner',
      threadId: 'thread',
      catId: sourceKind === 'owner' ? null : 'opus',
      mentions: ['codex'],
      extra: {},
    };
    const bundle = {
    ...
    assert.equal(registryArgs[4], sourceKind === 'a2a' ? source.id : undefined, 'owner calls must not impersonate A2A');
    assert.equal(registryArgs[6], source.id, 'callback identity retains the exact source message');
