...
export function registerCallbackHoldBallRoutes(app: FastifyInstance, deps: HoldBallRouteDeps): void {
  ...
  app.post('/api/callbacks/hold-ball', async (request, reply) => {
    ...
    const retiredPublishedProducerIds = new Set<string>();
    for (const prior of pendingHolds) {
      ...
      if (retirement === 'retired' || retirement === 'missing') {
        if (retirement === 'retired') retiredPublishedProducerIds.add(prior.id);
        priorHoldsToRemove.push(prior);
        continue;
      }
      ...
      try {
        ...
        const priorCommand =
          priorLifecycle?.mode === 'wake_when'
            ...
            : undefined;
        if (retiredPublishedProducerIds.has(prior.id)) {
          // Carrier retirement already consumed and disabled this producer.
          // Preserve that terminal receipt; prior.params is now a stale snapshot.
          taskRunner.unregister(prior.id);
        } else if (priorCommand) {
          const retiredParams = {
            ...prior.params,
            holdLifecycle: {
