  ...
  app.get('/api/threads', async (request, reply) => {
    ...
    if (featureIds) {
      const ids = featureIds
        ...
        .filter((id) => /^(?:f\d{2,4}|ext-\d{3})$/i.test(id));
      if (ids.length > 50) {
        reply.status(400);
      ...
      if (ids.length > 0) {
        ...
        for (const fid of ids) {
          if (fid.startsWith('ext-')) {
            const num = Number.parseInt(fid.slice('ext-'.length), 10);
            patternsByCanonical.set(fid.toUpperCase(), new RegExp(`\\bext[-\\s]*0*${num}(?!\\d)`, 'i'));
          } else {
