...
export class QueuedMessageCustodyStartupReconciler {
  ...
  /** Retire stale Review stage carriers before any restored Queue row becomes actionable. */
  private async withdrawSupersededReviewOrchestrationGroups(groups: Map<string, StoredMessage[]>): Promise<number> {
    ...
    for (const entryId of supersededReviewEntryIds(groups)) {
      ...
      try {
        ...
        // restored in this pass. A later startup pass can finish idempotently.
  ...
  private async withdrawReviewOrchestrationGroup(
    ...
    const entry = buildQueueEntry(messages, entryId);
