...
export class ReviewRoundStageDispatcher implements IReviewRoundStageDispatcher {
  ...
  private async deliver(
    ...
    const response = await this.deps.sendMessage({
      ...
      payload: {
        content: buildStageMessage(input, routing),
...
function buildStageMessage(input: ReviewRoundDispatchInput, routingHandles: readonly string[]): string {
  ...
  const authorization = input.consensusAuthorization
    ? [
        `用户裁决（${input.consensusAuthorization.authorizedByUserId} · ${new Date(input.consensusAuthorization.authorizedAt).toISOString()}）：`,
