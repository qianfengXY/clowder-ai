...
cited_by:
  ...
  - {feature: EXT-001, date: 2026-08-07, delta: "named Desktop development consumer requires ordered attempts, typed evidence, and terminal transitions; claims stay fail-closed until that F275 port exists"}
---
...
6. Phase B records no whole-work terminal state. The EXT-001-named Phase C extension owns only ordered attempt CAS, typed evidence, and accepted/rejected terminal state for that consumer; it adds no second producer, reopen command, generic event ledger, or repair subsystem.
...
- EXT-001 is the first named Phase C consumer. Its required port is read/validate work+attempt, idempotent next-attempt creation, typed evidence append, and F275-owned accepted/rejected transitions. EXT-001 may not implement a local fallback while this port is absent.
