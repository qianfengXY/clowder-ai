...
9. A Desktop wake remains in the durable outbox until the deterministic delivery ID or objective is observable in an actual bound-task turn; IPC acknowledgement or goal metadata alone is insufficient. After the owner acknowledges a start-turn request, recovery is verification-only and must not retransmit that message. An existing owner is never deep-linked merely to focus it; a dormant binding may be opened in place for bounded owner discovery and is never replaced.
