...
**Trigger:** 同一条 EXT-001 讨论链中两次把较窄作用域的规则提升成更广产品契约：先把 Cat Café fork 的 upstream 授权限制套到个人新项目，后把 Traqen 当前的 Review publication policy 套到通用 Desktop development loop。两次均由 operator 纠正。
...
3. `b6ecf8c8f..af10c401c`：EXT-001 从 GitHub Issue/token/publisher 核心依赖修正为 `ReviewRound -> MCP -> Desktop`，提供可复核的 artifact delta。
