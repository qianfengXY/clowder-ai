...
5. **Verification:** original regression run produced 12 failures. Independent review then reproduced a late cancellation during host exit (one failing test); retirement now checks cancellation after close as well. The corrected implementation passes 119 targeted tests across invocation, binding, managed-work consumption, host pool, transport, protocol, and pool registry. Independent review must cover the final PR head.
