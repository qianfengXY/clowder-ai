...
宪宪 / claude-opus-5 independently reviewed `9fe01ad71f47d99f3847e39c687716ece1dae236` in `/tmp/cat-cafe-review/workspace-live-refresh-pr27/xianxian`. Reverting only the real owner to main produced 9/12 regression failures; restoring it passed all 12. The reviewer also passed the 34-test affected set and the real browser scenario at `http://127.0.0.1:60832` with all eight displayed versions matching disk. This establishes reproduction and independent behavioral validation; it is not merge approval.

- **P1-1 (build):** the unused `_path` argument passed Biome but failed Next's ESLint `@typescript-eslint/no-unused-vars` rule. Author reproduced `next lint --file src/hooks/useWorkspaceFile.ts` exiting 1 with that error, then removed the unused parameter. TypeScript permits this callback to satisfy the existing path-taking caller. Production web build is now required evidence for this PR, in addition to typecheck and behavioral checks.
- **P3-1:** document and locally suppress the cleanup warning. `requestSeq` is a monotonically increasing counter, so cleanup must invalidate the current generation, not a captured numeric value. No rule is disabled outside that one line.
- **P3-2 / failure-mode sweep:** production `useWorkspace()` has one consumer, `WorkspacePanel`, which consumes worktree identity and search only. The remaining `tree`, `fetchTree`, `fetchSubtree`, `loading`, `error`, and `revealInFinder` exports are also unused there; the rendered tree is owned by `F307FilesOwnerSurface` and `useFileManagement`. They are existing redundant work, not a remaining document-body freshness path. This PR retires the orphan file lifecycle; tree cleanup is not claimed or required for the document acceptance criteria.
- **P3-3:** the focused browser script runs without the missing resource-lease wrapper. The standard `test` chain runs `test:unit` and `test:browser` first, so their existing wrapper import failure prevents that chain from reaching this regression. Run `test:workspace-live-refresh` explicitly until the runner is repaired; a green focused run is not a green `pnpm test`. TODO when `scripts/lib/process-resource-lease.mjs` is restored: route this script through `run-with-node-env-test.mjs` and verify browser resource admission; no substitute lease implementation is introduced here.
- **P3-4:** retain the initial connect revalidation. Mount and connection can cause two reads, but every connection uses the same recovery path independent of watcher notification delivery. Avoiding one startup GET is optional optimization and does not justify adding a separate first-connect state in this fix.
- **Descriptor identity observation:** explicit reopen changes the target descriptor object; layout actions preserve other descriptor identities. The reviewer inspected those reducer paths. No layout reducer changes are made by this PR; the actual owner reopen tests cover its freshness behavior, and no additional reducer-invariant coverage is claimed.
- **Hotfix checker observation:** the scoped `fix(workspace):` title is not recognized by the current checker. This PR still requires independent review and non-author merge; the checker is not used as an exemption. Its regex and default integration-base policy are separate harness concerns, not changed here.

Post-fix author evidence (2026-09-21):

- Next lint on all four changed production files: zero warnings/errors; affected 34 tests in five files passed; Biome and diff checks passed.
- `env -u NODE_ENV pnpm --filter @cat-cafe/web build`: exit 0, compiled successfully, lint/type validation completed, 44/44 static pages generated; postbuild production-cache-policy tests 2/2 passed. Existing unrelated warnings remain; none reference the four changed production files. Full log: `/tmp/workspace-live-refresh-build-r2.log`. This pre-commit dirty-checkout build intentionally has no clean build stamp and is not a deployment artifact.
- `env REDIS_URL=redis://localhost:6398 pnpm --filter @cat-cafe/web test:workspace-live-refresh`: 1/1 passed, eight body versions equal disk, with atomic rename, reconnect, same-path reopen and draft preservation all true. Fixture URL `http://127.0.0.1:53077`, shut down afterward; full log `/tmp/workspace-live-refresh-browser-r2.log`.
